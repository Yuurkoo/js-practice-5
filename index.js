...
function printStrings(array) {
  ...
  let doubleRow = row.repeat(2);

  console.log(doubleRow);
}
...
function concatenateStrings(array, subString) {
  ...
  let res = str.includes(subString);
...
function addTenToEach(array) {
  ...
  for (let index in array) {
    let mainAr = array[index];
    newAr[index] = mainAr + 10;
  }
...
function replaceNumbers(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] > 10) {
      let greater = "Greater than 10";
      newArray.push(greater);
    } else if (arr[i] <= 10) {
      let less = "Less than or equal to 10";
      newArray.push(less);
    }
  }
  return newArray;
